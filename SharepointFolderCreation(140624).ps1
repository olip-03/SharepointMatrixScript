...
$timestamp = Get-Date -Format "yyyy-MMM-dd-hhmmttss" 
$runLocation = Get-Location
$logfileName = "$runLocation\SharepointSiteSetup$timestamp.log"

New-Item $logfileName | Out-Null
Write-Host "Log file created at $logfileName" -ForegroundColor Yellow
    ...
    if ($ontoFileRows) {
        ...
        try {
            ...
            if ($null -ne $rowDataArray[0].Value) {
                ...
                $folderName = "$($rowDataArray[4].Value)".Trim()
                ...
                $exactTime = Get-Date -Format 'HH:mm:ss'
                $message = "[$exactTime] PASS [Folder]: Folder created at $folderLocation"
                    ...
                    try {
                        switch ($rowDataArray[$i].Value) {
                            "W" {     
                                ...
                                $exactTime = Get-Date -Format 'HH:mm:ss'
                                $message = "[$exactTime] PASS [Permissions]: WRITE access provided to $spGroupName"
                            ...
                            "R" {  
                                ...
                                $exactTime = Get-Date -Format 'HH:mm:ss'
                                $message = "[$exactTime] PASS [Permissions]: READ access provided to $spGroupName"
                                Write-Host $message -ForegroundColor DarkGreen
                        ...
                        $exactTime = Get-Date -Format 'HH:mm:ss'
                        $message = "[$exactTime] FAIL [Permissions]: Failed writing permissions for $($spGroupName): $_"
            ...
            $exactTime = Get-Date -Format 'HH:mm:ss'
            $message = "[$exactTime] FAIL [Folder]: Failed creating folder at $($folderLocation): $_";
            Write-Host $message -ForegroundColor Red
